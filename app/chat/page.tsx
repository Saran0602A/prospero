...
export default function Chatbot() {
  ...
  // ✅ Fetch logged-in user and sessions
  useEffect(() => {
    const getUser = async () => {
  ...
  // ✅ Load chat messages for selected session
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: "⚠️ Failed to connect to server. Please try again." },
      ]);
    } finally {
      setInput("");
      setLoading(false);
  ...
  if (!userId)
    return (
      <p className="p-6 text-center text-gray-600">
        🔒 Please sign in to chat with Prospero.
      </p>
    );
    ...
    <div className="flex min-h-screen bg-gradient-to-br from-green-100 to-blue-100">
      ...
      <div className="flex-1 flex flex-col h-screen">
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          ...
          {loading && (
            <p className="text-gray-500 animate-pulse mt-2">Typing...</p>
          )}
