...
import { MdThumbUpAlt, MdOutlineThumbUpAlt, MdOutlineComment, MdWork, MdSend, MdOutlineCategory, MdPinDrop, MdStarRate, MdAttachMoney, MdAccountCircle, MdSearch } from 'react-icons/md'
...
// --- Constants for Theme & Style ---
const PRIMARY_HEX = '#fca311' // Prospero Orange (Hope/Action)
const ACCENT_COLOR_TEXT = `text-[${PRIMARY_HEX}]`;
const PRIMARY_BG_COLOR = `bg-[${PRIMARY_HEX}]`;

// Typography (Impactful and Accessible)
...
const ProsperoBody = "text-base text-gray-700 leading-normal"; // For PostCard content


// --- UI Utility Components/Constants ---
// FIX: Custom motion Link component to avoid <a> nesting error
...
const CardWrapper = ({ children, className = '' }: { children: React.ReactNode, className?: string }) => (
    <motion.div
        ...
        className={`group bg-white p-6 rounded-2xl shadow-xl border border-gray-100 mb-6 transition-all duration-300 
...
const InputField = (props: any) => (
    <motion.input
        {...props}
        // General Input Field uses internal padding for clearance
...
const SearchInputField = (props: any) => (
    <motion.input
        {...props}
        // Specialized Search Input Field uses PL-12 padding for icon on the left
        className="w-full p-4 rounded-xl border-2 border-gray-200 focus:outline-none focus:ring-4 focus:ring-opacity-60 focus:ring-amber-400 transition duration-200 font-medium text-gray-800 placeholder-gray-500 pl-12" 
        whileFocus={{ boxShadow: `0 0 0 3px rgba(252, 163, 17, 0.6)` }}
    />
)
...
// --- AddPost Component (UPDATED for image_url) ---
function AddPost({ currentUser, onPost }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true)
      // Pass image URL to the parent handler
  ...
  return (
    <CardWrapper>
      ...
      <InputField
            type="url"
            placeholder="Paste Image URL here (e.g., proof of certification, community event photo)"
            value={imageUrl}
            onChange={e => setImageUrl(e.target.value)}
            className="mt-3 pl-4" // Override pl-12 for standard URL input look
        />
      ...
      <motion.button
        ...
        className={`w-full mt-4 px-4 py-3 text-lg font-bold text-white rounded-xl shadow-lg transition duration-200 ease-in-out bg-blue-600 hover:bg-blue-700 
...
// --- AddJobForm Component (Code omitted for brevity, assumed functional) ---
function AddJobForm({ currentUser, onJob }: any) {
  ...
  const handleSubmit = async () => {
    ...
    try {
        await onJob({
            title, description, category, location, pay, rating: rating || 0,
            deadline: deadline || null,
            posted_by: currentUser.id
        })
        setTitle(''); setDescription(''); setCategory(''); setLocation('');
        setPay(undefined); setRating(undefined); setDeadline(''); setLimit(undefined)
    } catch (err: any) {
  ...
  return (
    <CardWrapper>
      ...
      <div className="grid grid-cols-3 gap-3 mt-3">
        <InputField type="number" value={rating ?? ''} onChange={e => setRating(Number(e.target.value))} placeholder="Rating (0-5, optional)" className="pl-4" />
        ...
        <InputField type="number" value={limit ?? ''} onChange={e => setLimit(Number(e.target.value))} placeholder="Max Apps (Optional)" className="pl-4" />
      </div>
      ...
      <motion.button
        onClick={handleSubmit}
        disabled={loading || !title || !pay || !category || !location}
        style={{ backgroundColor: loading || !title || !pay || !category || !location ? '#9ca3af' : PRIMARY_HEX }}
...
// --- JobCard Component (Code omitted for brevity, assumed functional) ---
function JobCard({ job, currentUser, index }: any) {
  const [applied, setApplied] = useState(false)
  const [applicationsCount, setApplicationsCount] = useState(job.applications_count || 0)
...
  // Checking status based on the new schema: ('open','in_progress','completed')
...
  // User can apply if: not closed AND not already applied AND is NOT the job poster
  const canApply = !applied && !isClosed && !isPoster;

    // FIX: Define cardVariants within JobCard scope
    const cardVariants = {
...
      </div>
      
      <div className="grid grid-cols-2 gap-y-3 text-base text-gray-700 mb-6 font-medium border-b pb-4 border-gray-100">
...
        <p className="flex items-center"><MdAttachMoney className={`mr-2 ${ACCENT_COLOR_TEXT} text-lg`} /> <span className="font-bold">Applicants:</span> {applicationsCount} </p>
      </div>

      {canApply ? (
          // Link for redirection to application form
          <MotionLink 
              href={`/Apply/${job.id}`} 
              style={buttonStyle}
              className={`${buttonClassName} flex items-center justify-center shadow-lg`}
              whileTap={{ scale: 0.95 }}
          ...
          // Disabled button for applied/closed/self-posted jobs
          <motion.button
              disabled
              style={buttonStyle}
              className={`${buttonClassName} disabled:opacity-50 disabled:cursor-not-allowed shadow-md`}
              whileTap={{ scale: 1 }}
          >
              {buttonContent}
          </motion.button>
...
// --- PostCard Component (UPDATED to display image) ---
function PostCard({ post, currentUser, index }: any) {
  ...
  useEffect(() => {
    const checkLiked = async () => {
        if (!currentUser) return
        const { data } = await supabase.from('reactions').select('*').eq('post_id', post.id).eq('user_id', currentUser.id).single()
        setLiked(!!data)
    }
    checkLiked()
  }, [currentUser, post.id])
  ...
  const toggleLike = async () => {
    ...
    // FIX: Define cardVariants within PostCard scope
    const cardVariants = {
        hidden: { opacity: 0, x: -20 },
        visible: (i: number) => ({
            opacity: 1,
  ...
  return (
    ...
    >
      <div className="flex items-start mb-3">
        ...
        <div className="ml-3">
          <h3 className="font-extrabold text-lg text-gray-900 leading-snug tracking-tight">{post.user?.name || 'Anonymous User'}</h3>
      ...
      <div className="flex items-center gap-6 border-t pt-3">
        <motion.button 
            onClick={toggleLike} 
            className={`flex items-center gap-1.5 py-1 px-3 rounded-full transition-colors font-medium ${liked ? 'text-blue-600 bg-blue-50' : 'text-gray-500 hover:text-blue-500 hover:bg-gray-100'}`}
            whileTap={{ scale: 0.95 }}
        >
          {liked ? <MdThumbUpAlt className="text-xl" /> : <MdOutlineThumbUpAlt className="text-xl" />}
          Like
        </motion.button>
        <motion.button 
...
// --- Comments Section (Code omitted for brevity, assumed functional) ---
function CommentsSection({ postId, currentUser }: any) {
  ...
  const handleComment = async () => {
    ...
    // Add time formatting for the new comment
    const newComment = { 
        ...data[0], 
...
export default function FeedPage() {
  ...
  const [view, setView] = useState<'posts' | 'jobs'>('jobs') // Default view to Jobs for job-focused platform
  ...
  // --- Data Fetching Logic (Jobs and Posts) ---
  const { data: postsPages } = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage, pages) => (lastPage.length === 5 ? pages.length * 5 : undefined),
  })
  const allPosts = postsPages?.pages.flat() || []

  const { data: jobsPages } = useInfiniteQuery({
    queryKey: ['jobs', id],
    queryFn: async ({ pageParam = 0 }) => {
      const limit = 5
      const { data } = await supabase
        ...
        .range(pageParam, pageParam + limit - 1)
      return data?.map(job => {
        const applications_count = job.applications_count?.[0]?.count || 0 
        const isClosed = job.status !== 'open' || (job.deadline && new Date(job.deadline) < new Date());
        return { ...job, status: isClosed ? job.status : 'open', applications_count }
      }) || []
    },
    enabled: !!currentUser,
    getNextPageParam: (lastPage, pages) => (lastPage.length === 5 ? pages.length * 5 : undefined),
  ...
  const filteredJobs = useMemo(() => {
    ...
    return allJobs.filter(job => 
        ...
        job.posted_by?.name?.toLowerCase().includes(query) // Search by poster name
    );
  }, [allJobs, searchQuery]);
  ...
  const filteredPosts = useMemo(() => {
    ...
    return allPosts.filter(post => 
        ...
        post.hashtags?.some((h: any) => h.hashtag_id.tag.toLowerCase().includes(query)) ||
        post.user?.name?.toLowerCase().includes(query) // Search by posted user name
    );
  }, [allPosts, searchQuery]);


  // --- Add Post/Job Handlers (UPDATED to accept image URL) ---
  const handleNewPost = async (content: string, category?: string, hashtagsArr?: string[], imageUrl?: string) => {
    if (!currentUser) return
    // Include image_url in the insert payload
    ...
    queryClient.setQueryData(['posts', id], (old: any) => {
      ...
      pages[0] = [postWithUser, ...(pages[0] || [])]
      return { ...old, pages }
    })
  }

  const handleNewJob = async (jobData: any) => {
    if (!currentUser) return
    
    // Clean jobData to only include fields present in the 'jobs' schema
    const newJobData = {
      title: jobData.title,
    ...
    const { data } = await supabase.from('jobs').insert(newJobData).select()
    ...
    queryClient.setQueryData(['jobs', id], (old: any) => {
      const pages = old?.pages || []
      pages[0] = [newJob, ...(pages[0] || [])]
      return { ...old, pages }
    })
  }

// --- Left Column Profile Card Component (FINAL) ---
const ProfileCard = () => (
    <CardWrapper className="shadow-2xl border-gray-200 p-8 mr-4">
        ...
        <div className="space-y-4 text-base">
            ...
            <div className="flex justify-between items-center text-gray-700">
                ...
                <span className="font-extrabold text-lg">{currentUser?.rating || 'N/A'}</span>
            </div>
        </div>
  ...
  return (
    <div className="min-h-screen p-4 bg-stone-50 font-sans">
      <div className="max-w-7xl mx-auto flex gap-8"> {/* Increased gap for more space */}
          ...
          <sup className="text-[#14213d] font-bold text-xl">.</sup>
            ...
            <motion.div initial={{ y: -10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mb-6">
                <div className="relative flex items-center">
                    ...
                    />
                    {/* Icon positioned to the right end */}
                    <MdSearch className="absolute right-4 text-gray-500 text-2xl" />
                </div>
            </motion.div>
            
            {/* View Toggles (Professional, Accessible Tabs) */}
            <div className="flex gap-2 mb-8 p-1 rounded-2xl bg-white shadow-xl border border-gray-200">
              <motion.button
                onClick={() => {setView('jobs'); setSearchQuery('');}} // Reset search when switching tabs
                style={{ 
                    backgroundColor: view === 'jobs' ? PRIMARY_HEX : 'transparent',
                    color: view === 'jobs' ? 'white' : '#1f2937'
                }}
              ...
              <motion.button
                onClick={() => {setView('posts'); setSearchQuery('');}} // Reset search when switching tabs
                style={{ 
                    backgroundColor: view === 'posts' ? PRIMARY_HEX : 'transparent',
                    color: view === 'posts' ? 'white' : '#1f2937' 
                }}
                className={`flex-1 px-6 py-4 rounded-xl font-bold text-lg transition duration-200 ease-in-out shadow-lg`}
                whileTap={{ scale: 0.98 }}
                whileHover={{ scale: 1.01 }}
            ...
            >
              {view === 'jobs' && (
                <>
                  {/* Employer posts form ONLY if user is an employer. */}
                  ...
                  {filteredJobs.length === 0 && (
                    <p className="text-center text-gray-500 mt-10 p-6 bg-white rounded-2xl shadow border border-gray-200">
                        {searchQuery ? `No jobs found matching "${searchQuery}".` : "No current job openings match your profile."}
                    </p>
                  )}
                </>
              )}
